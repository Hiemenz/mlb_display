...
from image_win_trend import render_win_trend_view, render_all_teams_win_trend_view, render_divisions_win_trend_view

_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_DEFAULT_OUTPUT = os.path.join(_REPO_ROOT, 'resulting_image.bmp')


_VALID_MODES = ('scoreboard', 'linescore', 'scorecard', 'derby', 'fields', 'quadrant', 'race', 'wintrend', 'wintrend_all', 'wintrend_div')
...
def render(config, date_str=None, output_path=None, bypass_cache=False):
    ...
    if mode == 'wintrend_div':
        all_trend_data = load_json_file('all_teams_trend.json')
        if not all_trend_data:
            print("No all_teams_trend.json — run src/fetch_win_trend.py --all")
            return None
        image = render_divisions_win_trend_view(all_trend_data, config=config,
                                               dark_mode=config.get('dark_mode', False))
        if output_path:
            image.save(output_path)
            print(f"Image saved to {output_path}")
